...
[source,sh,role=execute]
----
rosa create admin --cluster rosa-${GUID} > ${HOME}/login-command.txt
...
----
cd $HOME
wget -q https://raw.githubusercontent.com/openshift-cs/rosaworkshop/master/rosa-workshop/ostoy/yaml/ostoy-microservice-deployment.yaml
curl -s https://raw.githubusercontent.com/0kashi/rosaworkshop/master/rosa-workshop/ostoy/yaml/ostoy-frontend-deployment.yaml | sed 's/\# serviceAccount/serviceAccount/' > ${HOME}/ostoy-frontend.deployment.yaml
